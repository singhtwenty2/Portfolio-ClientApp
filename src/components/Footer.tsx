...
import { Github, Linkedin, Mail, ChevronUp, ExternalLink } from 'lucide-react';
...
const FooterLink = ({ link }: { link: { name: string; path: string; icon?: React.ReactNode } }) => {
    const router = useRouter();
    const isExternal = link.path.startsWith('http');
    const baseStyles = "flex items-center gap-2 text-sm tracking-wide font-light text-zinc-400 hover:text-white transition-all duration-700 relative z-10 group";
    ...
    const content = (
        <span className="relative flex items-center gap-2">
            {link.name}
            {isExternal && <ExternalLink className="w-3 h-3 opacity-50 group-hover:opacity-100 transition-opacity duration-700" />}
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-16 sm:pt-20 pb-8 sm:pb-12 relative z-10">
                ...
                >
                    ...
                    <div className="flex justify-center space-x-6">
                        <Link href="/privacy-policy" className="text-sm text-zinc-400 hover:text-white transition-colors duration-700">
                            Privacy Policy
                        </Link>
                        <span className="text-zinc-700">|</span>
                        <Link href="/terms-and-conditions" className="text-sm text-zinc-400 hover:text-white transition-colors duration-700">
                            Terms & Conditions
                        </Link>
                    </div>
