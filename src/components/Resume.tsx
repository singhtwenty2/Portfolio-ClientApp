...
const DownloadToast: React.FC = () => (
    ...
    >
        <div className="flex-1">
            <h3 className="font-medium text-zinc-100 mb-1">Resume Download Initiated</h3>
            <p className="text-sm text-zinc-400">Your file will be ready momentarily...</p>
        </div>
        <motion.div
...
const FloatingButton: React.FC = () => {
    ...
    const handleDownload = async () => {
        ...
        try {
            ...
            link.href = pdfUrl;
            link.target = '_blank';
            link.download = 'Aryan_Singh_Resume.pdf'; // The name the file will be downloaded as
        ...
        <div className="min-h-screen w-full bg-gradient-to-b from-black via-zinc-950 to-black relative overflow-hidden">
            ...
            <div className="relative w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 pb-32">
                ...
                >
                    ...
                    >
                        <h1 className="text-4xl font-bold text-zinc-100 mb-4">{data.name}</h1>
                        <div className="flex flex-wrap justify-center gap-4 text-sm text-zinc-400">
                            <a href={`mailto:${data.contact.email}`} className="hover:text-blue-400 transition-colors">
                                {data.contact.email}
                            </a>
                            <span>{data.contact.phone}</span>
                            <a href={data.contact.linkedin} target="_blank" rel="noopener noreferrer" className="hover:text-blue-400 transition-colors">
                                LinkedIn
                            </a>
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {data.technicalSkills.map((skill, index) => (
                                <div key={index} className="p-4 bg-zinc-900/30 rounded-xl border border-zinc-800/50">
                                    <h3 className="font-semibold text-zinc-100 mb-2">{skill.category}</h3>
                            ...
                            {data.projects.map((project, index) => (
                                ...
                                >
                                    <div className="flex flex-wrap items-baseline gap-2 mb-2">
                                        ...
                                        <span className="text-zinc-400 text-sm">{project.duration}</span>
                                    </div>
                                    <ul className="list-disc pl-5 space-y-2 text-zinc-300">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {data.achievements.map((achievement, index) => (
                                ...
                                >
                                    <h3 className="font-semibold text-zinc-100 mb-2">{achievement.title}</h3>
                        ...
                        <div className="space-y-6">
                            {data.education.map((edu, index) => (
                                ...
                                >
                                    ...
                                    <p className="text-zinc-300">{edu.institution} — {edu.score} — {edu.duration}</p>
                                </div>
                            ))}
                        </div>
                    </motion.div>
